...
void SemanticAnalysis::exitConst_Var_Def(T1 *ctx, bool is_const){
    ...
    if(ctx->constInitVal()!=nullptr){
        //同时考虑非数组左右都应当为0
        if(ctx->arraydims.size() != ctx->constInitVal()->dim_index){ // 两侧dim都为0
        ...
        #ifdef IR_GEN
        //arg1表示初始值串
        std::string initval = IMM_PREFIX + ctx->constInitVal()->value_list; // 变量声明初始化
        addIRC( IRop,
                ctx->basety,
                IR_name,
                initval);
...
void SemanticAnalysis::enterCompUnit(CACTParser::CompUnitContext *ctx){ //begin
    //初始化根作用域及当前作用域
    ...
    addBuiltinFunc("print_char", 1, BTY_CHAR, BTY_VOID);
    ...
    addBuiltinFunc("get_char", 0, BTY_VOID, BTY_CHAR);
}
void SemanticAnalysis::exitCompUnit(CACTParser::CompUnitContext *ctx){ // 
    ...
    //获取表项
    auto item = iter->second; // 键值对第二项<func_symbol_item_t>
...
void SemanticAnalysis::exitConstInitVal(CACTParser::ConstInitValContext *ctx){ // //?
    if(ctx->constExp() != nullptr){ // 非数组dim=0
        ctx->dim_index = 0;
        #ifdef IR_GEN
            ctx->value_list = ctx->constExp()->valstr + ","; //g4
        #endif
    }
    else{
        //检查是否存在维度数组
        ...
        auto initval_list = ctx->constInitVal();//{}中长度
        ...
        if(len == 0){
            ...
            if(ctx->top == true && ctx->dim_index == 1){ // a[2] = {1,2}
...
void SemanticAnalysis::exitFuncFParam(CACTParser::FuncFParamContext *ctx){
    ...
    //根据括号判断总层数
    int dim_layer = (ctx->LeftBracket()).size(); // antlr自动检测左括号数量
...
void SemanticAnalysis::enterLab(CACTParser::LabContext *ctx){ // IR
    #ifdef IR_GEN
    addIRC( IR_LABEL,
            BTY_UNKNOWN,
            ctx->in_label);
...
void SemanticAnalysis::exitLab(CACTParser::LabContext *ctx){} // IR

void SemanticAnalysis::enterGo(CACTParser::GoContext *ctx){ // IR
    #ifdef IR_GEN
    addIRC( IR_J,
            BTY_UNKNOWN,
            ctx->out_label);
...
void SemanticAnalysis::exitGo(CACTParser::GoContext *ctx){} // IR
...
void SemanticAnalysis::enterBlock(CACTParser::BlockContext *ctx){ // 
        ...
        for(int i=0;i<(*ctx->fparam_list_ptr).size();i++){
            ...
            if(fparam.type.arrdims.size()!=0){
                switch(fparam.type.basety){
                    ...
                    case BTY_CHAR: basety = BTY_CHAR_PTR; break;
                    default: break;
                }
            }
            else{
            ...
            (*ctx->fparam_list_ptr)[i].IR_name = IR_name; // symbol_table
    ...
    #ifdef IR_GEN // 继承控制label
    for(auto stmt:ctx->stmt()){
        stmt->break_label = ctx->break_label; // g4
...
void SemanticAnalysis::exitStmt_assign(CACTParser::Stmt_assignContext *ctx){ //
    ...
    //类型检查，创建临时表达式以应用框架
    cact_op_t op = OP_ASSIGN;//(typeutils.str_to_op)[ctx->ASSIGN()->getText()];
    subexprs_t subexprs;
...
void SemanticAnalysis::enterStmt_block(CACTParser::Stmt_blockContext *ctx){ // 
    ...
    #ifdef IR_GEN
    ctx->block()->break_label = ctx->break_label; // g4
...
void SemanticAnalysis::enterStmt_if(CACTParser::Stmt_ifContext *ctx){ // if not ir no op
    #ifdef IR_GEN
    ...
    if(branch == 1){ //if
        ...
        ctx->cond()->true_label = if_start; // g4
        ctx->cond()->false_label = end_label;
        ctx->lab()[0]->in_label = if_start; // g4
...
void SemanticAnalysis::enterStmt_while(CACTParser::Stmt_whileContext *ctx){ // if not ir no op
    #ifdef IR_GEN //g4
...
void SemanticAnalysis::exitStmt_bcr(CACTParser::Stmt_bcrContext *ctx){ //
    ...
    if(ctx->RETURN()!=nullptr){
        ...
        else{ //return语句负责将变量存到a0，exp为空时不需要 return
            ctx->ret_type = BTY_VOID;
        }
    }
    else{
...
void SemanticAnalysis::exitExp(CACTParser::ExpContext *ctx){ // 没有bool本段只需要最后
    //ctx->self = ctx->lOrExp()->self;
    ctx->self = ctx->addExp()->self;
    #ifdef IR_GEN
    //ctx->result_name = ctx->lOrExp()->result_name; // g4 addExp
...
void SemanticAnalysis::exitConstExp(CACTParser::ConstExpContext *ctx){ //
    ...
    if(ctx->number() != nullptr){
        ...
        #ifdef IR_GEN
        ctx->valstr = ctx->number()->getText(); // g4
        #endif
    }
}

void SemanticAnalysis::enterCond(CACTParser::CondContext *ctx){ // if not ir no op
    #ifdef IR_GEN
    ctx->lOrExp()->true_label = ctx->true_label;
    ctx->lOrExp()->false_label = ctx->false_label; // g4
    #endif
}
void SemanticAnalysis::exitCond(CACTParser::CondContext *ctx){ // int?
    //std::cout << ctx->getText() << std::endl;
    auto expr_ptr = ctx->lOrExp()->self;
    if((expr_ptr->basety != BTY_BOOL && expr_ptr->basety != BTY_INT) || expr_ptr->op == OP_ARRAY){
        std::cout << "Err: Cond: cond must be single bool / int" << std::endl;
        exit(1);
    }
}
...
void SemanticAnalysis::exitLVal(CACTParser::LValContext *ctx){
    ...
    if(layer == 0){
        ...
        //判断类型
        //int arr[3][4];
        //arr[1][2];  // 处理为 OP_ITEM (int)
        //arr[1];     // 处理为 OP_ARRAY (int[4])
        if(layer == dim_size){
            op = OP_ITEM;
        }
        else{
...
void SemanticAnalysis::exitPrimaryExp(CACTParser::PrimaryExpContext *ctx){ // 
    if(ctx->number()!=nullptr){
        ...
        #ifdef IR_GEN
        ctx->result_name = IMM_PREFIX + ctx->number()->getText(); // g4
        #endif
    }
...
void SemanticAnalysis::exitUnaryExp(CACTParser::UnaryExpContext *ctx){
    if(ctx->primaryExp()!=nullptr){
        ctx->self = ctx->primaryExp()->self; // 直接继承
        #ifdef IR_GEN
        ...
        if(iter == (symbol_table.func_table).end()){
            ...
            exit(1);
        }// undefinedFunc(); // Err: Called Func undefinedFunc Undefined
        ...
        if(ctx->funcRParams() == nullptr){
            ...
            //长度检查
            //std::cout << "exp_list.size() = " << exp_list.size() << " fparam_len = " << fparam_len << std::endl;
            if(exp_list.size() != fparam_len){
                std::cout << "Err: UnaryExp: param_len mismatched" << std::endl;
                exit(1);
            }
            //int foo(int a, int b);
            //foo(1); // Err: param_len mismatched

            //类型检查
            for(int i=0;i<fparam_len;i++){
                auto fparam_type = fparam_list[i].type;
                #ifdef IR_GEN
                std::string expr_name = exp_list[i]->result_name; // g4
                #endif
                auto expr_ptr = exp_list[i]->self;
                assert(expr_ptr);
                //基本类型检查
                ...
                //void bar(float arr[3][4]);
                //int matrix[2][3];
                //bar(matrix); // Err: param basety mismatched (int vs float)
                    ...
                    //void test(int arr[][4]);
                    //int data[3][5];
                    //test(data); // Err: param dim_len mismatched (5 vs 4)
...
void SemanticAnalysis::enterLAndExp(CACTParser::LAndExpContext *ctx){ // not ir
    #ifdef IR_GEN
...
void SemanticAnalysis::enterLOrExp(CACTParser::LOrExpContext *ctx){ // not ir
    #ifdef IR_GEN
    if(ctx->lOrExp()!=nullptr){ //lOrExp OR lAandExp
        //lOrExp部分出错需要跳转到lAndExp前
        std::string or_label = newLabel();
        ctx->lab()->in_label = or_label; //g4
        ctx->lOrExp()->true_label = ctx->true_label;
